...
void write_buffer8(u8 adr, u8 *data,uint8_t f)
{
	...
	SPI2_Send_byte(adr|0x40);//地址 地址自動加1
  for(i=0;i<f;i++)
...
u8 read_buffer(u8 adr)
{
	...
	SPI2_Send_byte(adr|0x80);//1000 0000讀取地址
...
void read_buffer8(u8 adr, u8 *data,uint8_t f)
{
	...
	SPI2_Send_byte(adr|0xc0);//1100 0000讀取地址自動加1
  for(i=0;i<f;i++)
...
{	 
  ...
  uint8_t i;
   ...
   read_buffer8(OUT_X_L,BUF,6);//讀取 x y z 的數據
   
   fs=read_buffer(CTRL_REG4);//讀取 reg4的數據
   c1=read_buffer(CTRL_REG1);  //測試SPI通信有沒有成功
   //printf("c1=%d \r\n", c1 );
